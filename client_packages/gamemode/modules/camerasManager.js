...
class CamerasManager {
    ...
    static setActiveCameraWithInterpExt(activeCamera, position, rotation, duration, easeLocation, easeRotation, fov = 40, dofSettings = undefined) {
        if (this.doesExist(CamerasManagerInfo.activeCamera)) {
            CamerasManagerInfo.activeCamera.setActive(false);
        }
        ...
        if (activeCamera._pointAtPos != undefined)
            activeCameraPointAt = activeCamera._pointAtPos;

		interpCamera.pointAtCoord(activeCameraPointAt.x, activeCameraPointAt.y, activeCameraPointAt.z);

		if (typeof(dofSettings) !== 'undefined') {
			if (dofSettings.shallowMode !== undefined) activeCamera.setUseShallowDofMode(dofSettings.shallowMode);
			if (dofSettings.nearDof !== undefined) activeCamera.setNearDof(dofSettings.nearDof);
			if (dofSettings.farDof !== undefined) activeCamera.setFarDof(dofSettings.farDof);
			if (dofSettings.strength !== undefined) activeCamera.setDofStrength(dofSettings.strength);
		}

        activeCamera.setCoord(position.x, position.y, position.z);
        activeCamera.setFov(fov);

        activeCamera.pointAtCoord(rotation.x, rotation.y, rotation.z);
        activeCamera._pointAtPos = rotation;
...
global.CameraManager = CamerasManager;